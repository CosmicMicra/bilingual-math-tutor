...
export default function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (uploadedImage) {
      ...
      setLoading(true);
      try {
        const result = await ocrTranslateProblem(uploadedImage, (status, progress) => {
          setOcrStatus(status);
          setOcrProgress(progress);
        });
        setOcrStatus(null);
        setOcrProgress(0);
        await processProblemText(result.original, null);
        clearImage();
      } catch (error) {
        console.error(error);
        setFeedback({ type: 'error', message: 'OCR failed. Please try a clearer image.' });
        setOcrStatus(null);
        setOcrProgress(0);
      } finally {
        setLoading(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="relative">
                <textarea
                  ...
                  maxLength={500}
                  placeholder={isPracticeMode ? 'Practice Mode pulls a question automatically...' : 'Type, speak, or upload an image of a math problem...'}
